...
describe('tool path normalization', () => {
  it('rejects paths outside the active workspace', () => {
    expect(() => resolveWorkspacePath('..\\outside.txt', {
      workspace: 'C:\\workspace\\course'
    } as never)).toThrow('Path is outside the active workspace')
  })

  it('resolves paths inside the active workspace', () => {
    expect(resolveWorkspacePath('slides\\lesson.md', {
      workspace: 'C:\\workspace\\course'
    } as never)).toMatchObject({
      workspaceRoot: 'C:\\workspace\\course',
      absolutePath: 'C:\\workspace\\course\\slides\\lesson.md',
      relativePath: 'slides\\lesson.md'
    })
  })
