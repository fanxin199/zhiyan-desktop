...
export function Workbench(): ReactElement {
  ...
  const {
    ...
    useShallow((s) => ({
      ...
      openSyllabus: s.openSyllabus,
      openPptGen: s.openPptGen,
      openPaperPolish: s.openPaperPolish,
      openLiterature: s.openLiterature,
      openTextbook: s.openTextbook,
      openBioinformatics: s.openBioinformatics,
      openFileManager: s.openFileManager,
  ...
  const isZhiYanModuleRoute = ['dashboard', 'syllabus', 'ppt-gen', 'paper-polish', 'literature', 'textbook', 'bioinformatics', 'file-manager'].includes(route)

  const handleModuleQuickPrompt = (prompt: string, options?: { workspaceRoot?: string }): void => {
    setInput(prompt)
    setRoute('chat')
    if (options?.workspaceRoot) {
      void createThread({ workspaceRoot: options.workspaceRoot })
    } else {
      void createThread()
    }
  }
      ...
      {!leftSidebarCollapsed ? (
        <>
          <div className="min-h-0 shrink-0" style={{ width: leftSidebarWidth }}>
            ...
            <ZhiYanSidebar
              activeRoute={
                isZhiYanModuleRoute ? route
                : route === 'chat' || route === 'claw' ? 'chat'
                : route
              }
