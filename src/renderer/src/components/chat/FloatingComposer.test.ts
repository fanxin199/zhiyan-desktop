...
describe('FloatingComposer capability controls', () => {
  ...
  it('keeps uploaded attachments visible without dashboard-only chat actions', () => {
    const html = renderToStaticMarkup(
      createElement(FloatingComposer, {
        input: 'describe this',
        setInput: () => undefined,
  ...
  it('hides the plan badge outside the chat route', () => {
    const html = renderToStaticMarkup(
      createElement(FloatingComposer, {
        input: 'plan this',
        setInput: () => undefined,
    ...
    expect(html).not.toContain('>Plan</span>')
  })
