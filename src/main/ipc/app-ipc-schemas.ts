...
export const coursewarePdfRangePayloadSchema = z.object({
  path: trimmedString(MAX_PATH_LENGTH),
  pageStart: z.number().int().positive().max(100_000),
  pageEnd: z.number().int().positive().max(100_000)
}).strict().refine((value) => value.pageEnd >= value.pageStart, {
  message: '结束页不能小于起始页。',
  path: ['pageEnd']
})

export const coursewarePdfInspectPayloadSchema = z.object({
  path: trimmedString(MAX_PATH_LENGTH)
}).strict()
...
export const pubMedSearchPayloadSchema = z.object({
  ...
  minYear: z.number().int().min(1900).max(2200).optional()
}).strict()

const coursewareGenerationBaseSchema = z.object({
...
export const coursewareBlueprintGenerationPayloadSchema = coursewareGenerationBaseSchema

export const coursewareSlidesGenerationPayloadSchema = z.object({
  request: coursewareRequestSchema,
  blueprint: coursewareBlueprintSchema,
  sourceText: z.string().trim().min(1).max(500_000),
  evidence: z.array(evidenceRefSchema).max(100).optional(),
  generationId: trimmedString(160).optional()
}).strict()

export const coursewareSlideRegenerationPayloadSchema = z.object({
  request: coursewareRequestSchema,
  blueprint: coursewareBlueprintSchema,
  sourceText: z.string().trim().min(1).max(500_000),
  evidence: z.array(evidenceRefSchema).max(100).optional(),
  slide: coursewareSlideSpecSchema,
  instruction: trimmedString(4_000)
}).strict()

export const coursewareExportPayloadSchema = z.object({
