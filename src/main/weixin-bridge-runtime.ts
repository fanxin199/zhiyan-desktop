...
function readWeixinPackageInfo(): WeixinPackageInfo {
  ...
  if (!packageJson) {
    throw new Error(
      'Built-in WeChat login component is missing. Reinstall ZhiYan Assistant or rebuild with @tencent-weixin/openclaw-weixin bundled.'
    )
  }
...
async function postToDeepSeekGuiWebhook(message: WeixinMessage, accountId: string): Promise<JsonRecord> {
  ...
  if (!res.ok || data.ok === false) {
    throw new Error(recordString(data, 'message') || `ZhiYan Assistant webhook HTTP ${res.status}`)
  }
  return data
}
