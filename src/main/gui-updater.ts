...
let configuredChannel: GuiUpdateChannel = normalizeGuiUpdateChannel(
  process.env.ZHIYAN_UPDATE_CHANNEL?.trim()
)
...
function envUpdateUrl(channel: GuiUpdateChannel): string {
  const channelSpecific = process.env[`ZHIYAN_UPDATE_URL_${channel.toUpperCase()}`]?.trim()
  const direct = channelSpecific || process.env.ZHIYAN_UPDATE_URL?.trim() || ''
...
function updateFeedUrl(channel: GuiUpdateChannel): string {
  ...
  throw new Error(
    `Automatic updates are not configured. Set ZHIYAN_UPDATE_URL_${channel.toUpperCase()} when a release service is available.`
  )
}
