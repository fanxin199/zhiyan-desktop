...
import zhiyanLogoPng from '../asset/img/zhiyan.png'
import {
...
const startupTraceEnabled = process.env.ZHIYAN_STARTUP_TRACE === '1'
...
function trayLabels(locale: AppSettingsV1['locale']): { show: string; quit: string; tooltip: string } {
  ...
  return {
    show: 'Show ZhiYan Assistant',
    quit: 'Quit',
    tooltip: 'ZhiYan Assistant'
  }
}
...
function syncLoginItemSettings(settings: AppSettingsV1): void {
  ...
  try {
    ...
    console.warn('[zhiyan] failed to update login item settings:', error)
...
async function showTurnCompleteNotification(
  ...
  const title = normalizeNotificationText(payload.title, '智研助手', 80)
...
async function ensureKunRuntime(settings: AppSettingsV1): Promise<void> {
  ...
  try {
    ...
    console.error('[zhiyan] failed to start kun:', e)
    throw e
  }
...
app.whenReady().then(async () => {
  ...
  const message = error instanceof Error ? error.message : String(error)
  console.error('[zhiyan] startup failed:', error)
...
app.on('window-all-closed', () => {
  void stopManagedRuntimes().catch((error) => {
    console.warn('[zhiyan] failed to stop Kun runtime:', error)
  })
