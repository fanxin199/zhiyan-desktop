...
export async function runClawScheduleMcpServerFromArgv(argv: string[]): Promise<boolean> {
  ...
  const registerListTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. List scheduled tasks managed by the currently running ZhiYan Assistant app.'
        : 'List scheduled tasks managed by the currently running ZhiYan Assistant app.'
    }, async () => {
      try {
  ...
  const registerCreateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Create a scheduled task in ZhiYan Assistant. Supports one-time (`at`), daily, or interval schedules.'
        : 'Create a scheduled task in ZhiYan Assistant. Supports one-time (`at`), daily, or interval schedules.',
      inputSchema: {
  ...
  const registerUpdateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Update an existing ZhiYan Assistant scheduled task.'
        : 'Update an existing ZhiYan Assistant scheduled task.',
      inputSchema: {
  ...
  const registerDeleteTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Delete a scheduled task from ZhiYan Assistant.'
        : 'Delete a scheduled task from ZhiYan Assistant.',
      inputSchema: {
        task_id: z.string().min(1).describe('Task id returned by gui_schedule_list or gui_schedule_create')
      }
    }, async ({ task_id }) => {
