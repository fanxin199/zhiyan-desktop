...
- [x] **1.2.1** 在 `ZhiYanDashboard.tsx` 中、问候语下方、"教学工具"卡片之前，增加"最近使用"区域。
  - 从 chat store 的 `threads` 数组读取最近 5 个非归档对话，按 `updatedAt` 倒序。
  - 每项显示：对话标题、关联模块图标、相对时间。
  - 点击可直接恢复到该对话。
- [x] **1.2.2** 如果没有任何历史对话，该区域不渲染（而不是显示空列表）。
...
- [x] **1.4.1** 在 `SettingsSidebar.tsx` 中的"通用"设置按钮下方增加说明文字或子项，提示"通用设置中可修改教师信息"。（此处不加新的侧边栏 tab，避免增加层级）
...
- [x] **1.5.5** 为上述模块增加会话隔离测试，确保切换到 AI 对话或其他模块后，不会在原模块误显示无关消息。
...
- [x] **2.1.4** 文件管理 AI 整理面板与安全执行流：在文件管理页增加本页 AI 整理面板，支持“分类整理、规范命名、查找重复文件、转换格式、提取摘要”等快捷任务；AI 先生成操作方案，涉及移动、删除、重命名、覆盖或批量修改时必须展示影响范围并等待老师确认后执行。

### Task 2.2 · 跨模块信息共享

- [x] **2.2.1** 在 `electron-store` 中增加 `moduleContext` 存储，按 projectId 索引，保存各模块最近使用的关键参数（如教案模块的课程名称、课件模块的教材标题等）。
- [x] **2.2.2** 当老师在"教案"模块填写了课程信息后，在"课件 PPT"模块自动读取并预填充相同课程信息。
- [x] **2.2.3** 在"文本写作"模块建立的 Blueprint 数据，在"综述撰写"模块可通过"导入已有蓝图"按钮引用。

### Task 2.3 · Dashboard 快速搜索

- [x] **2.3.1** 在 `ZhiYanDashboard.tsx` 的问候语区域下方增加一个轻量输入框，placeholder 为"输入关键词搜索功能，或直接描述需求…"。
- [x] **2.3.2** 输入文字时实时过滤 Dashboard 中的功能卡片（前端过滤，不需要后端）。
- [x] **2.3.3** 按回车时将输入作为自然语言 prompt 发送到 AI 对话界面。
...
- [x] **3.1.1** 在 `CoursewarePage.tsx` 顶部增加横向步骤条组件：① 上传教材 → ② AI 分析 → ③ 图片审核 → ④ 生成课件。根据当前阶段高亮对应步骤。
...
- [x] **3.2.4** 为文件列表项增加右键菜单："用于生成教案"、"用于文献精读"等跨模块材料入口。

### Task 3.4 · 科研数据分析模块降低门槛

- [x] **3.4.1** 在 `BIOINFORMATICS_CONFIG` 的 `taskEntry` 区域上方增加可折叠的"数据格式说明"区域，用简明图文解释"表达矩阵"、"分组表"等概念。

### Task 3.5 · 模块间区分提示

- [x] **3.5.1** 在"综述撰写"模块顶部增加一行灰色小字："本模块专注于综述文章撰写。如需修改已有论文或基金，请使用「文本写作」模块。"
- [x] **3.5.2** 在"文本写作"模块顶部增加："如需专门撰写国自然申请书，推荐使用「自然基金撰写」模块，内置国自然各模块模板。"

### Task 3.6 · 文献模块增强

- [x] **3.6.1** 将"文献汇报 PPT 大纲"的 `label` 改为"组会汇报 PPT 大纲"。
- [x] **3.6.2** 在 LITERATURE_CONFIG 的文件选择区域增加拖拽上传支持（监听 `onDragOver`/`onDrop` 事件）。
