...
**面向生物医学及相关专业大学老师的教学与科研 AI 工作台。无需代码基础，也能完成备课、课件、文献、写作和科研数据分析，并可自由选择 DeepSeek、Kimi、通义千问或智谱 GLM。**
...
- **模型选择更自由**：首次启动时选择 DeepSeek、Kimi、通义千问或智谱 GLM，填写对应 API Key 即可；服务地址和推荐模型自动配置。
...
| 科研数据分析 | 处理整理后的表达矩阵、差异结果、单细胞对象和常规统计表格，完成统计、可视化与生物医学解释 | 软件内管理分析引擎，记录输入、参数、代码、包版本和输出文件 |
...
2. 首次启动时选择 DeepSeek、Kimi、通义千问或智谱 GLM，并填写对应 API Key；软件只在本机保存配置信息。
...
当前版本为个人自用 Alpha，已经完成 700 余项自动化测试以及 Windows 成品结构和完整性检查。自动更新暂未启用；正式公开分发前仍需配置 Windows 代码签名并完成更多真实教师设备验证。
